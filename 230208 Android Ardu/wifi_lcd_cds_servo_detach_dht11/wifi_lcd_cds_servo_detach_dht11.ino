...
#define SERVER_NAME "10.10.141.9" //IP주소
#define SERVER_PORT 5001 // port 값
#define LOGID "LHG_ARD" //아두이노 id
...
char getSensorId[10]; //id 조정을 위해 배열 생성
int sensorTime;
float temp = 0.0;
float humi = 0.0;

DHT dht(DHTPIN, DHTTYPE);
SoftwareSerial wifiSerial(WIFIRX, WIFITX); //와이파이 시리얼 초기화
WiFiEspClient client; //클라이언트 통신
...
void setup() {
  lcd.init();
  lcd.backlight();
  lcdDisplay(0, 0, lcdLine1); // 0(첫번째) = x, 0(두번째) = y
  ...
  MsTimer2::set(1000, timerIsr); // 1000ms period 서버모트가 타이머 1을써서 2사용/ 1초마다 호출
  MsTimer2::start();

  myservo.attach(SERVO_PIN); 
  myservo.write(0); //값을 0으로 초기화
  myservoTime = secCount; // 1초 지나면 원하는 각도로 회전 
  dht.begin(); //온습도 초기화
}
...
void loop() {
  ...
  if (timerIsrFlag) //1초에 한번씩 실행
  {
    ...
    if (!(secCount % 5)) //5초에 한번씩 실행
    {
      ...
      Serial.println(temp);
#endif
      if (!client.connected()) { //5초마다 서버 종료 됐는지 확인
        lcdDisplay(0, 1, "Server Down");
        server_Connect(); //재연결 시도
      }
    }
    ...
    if (sensorTime != 0 && !(secCount % sensorTime )) //초에 배수에 따라 트루
    {
...
void socketEvent()
{
  ...
  if ((strlen(pArray[1]) + strlen(pArray[2])) < 16) //lec 16칸이므로 거기까지만 뿌리고 아니면 안뿌림 / 버퍼오버플로 조심 디버깅해도 안나옴
  {
...
void wifi_Setup() {
  wifiSerial.begin(19200); // 속도 넣기
  wifi_Init(); //
  server_Connect(); //
}
void wifi_Init()
{
  do { //do while 통해 꾸준히 연결
...
#endif
  while (WiFi.begin(AP_SSID, AP_PASS) != WL_CONNECTED) { //ap pass 통해 접속
#ifdef DEBUG_WIFI
    Serial.print("Attempting to connect to WPA SSID: ");
    Serial.println(AP_SSID);
#endif
